...
function initCogimixDeezer() {
	...
	DZ.Event.subscribe('player_play', function(evt_name) {
		...
		musicPlayer.bindCursorStop(function(value) {
			...
			try {
				DZ.player.seek(value * 100
						/ musicPlayer.plugin['dz'].currentDuration);
			} catch (err) {
				loggerDeezer.error(err);
			}
		});
	...
	DZ.Event.subscribe('current_track', function(args, evt_name) {
		...
		musicPlayer.cursor.slider("option", "max",
				musicPlayer.plugin['dz'].currentDuration).progressbar();
	});
...
function callLogin(loginLink) {
	DZ.login(function(response) {
		if (response.authResponse) {
			var postData = response;
			$.post(Routing.generate('_deezer_login_success'), postData,
					function(response) {
						if (response.success) {
							loginLink.replaceWith(response.html);
							$("#dz-playlist-container").empty();
							$("#dz-playlist-container").replaceWith(
									response.data.playlistsHtml);
							$(".dz-playlist-item").draggable(
									draggableOptionsPlaylistListItem);
						}
					}, 'json');
		}
...
function callLogout(logoutLink) {
...
$(document).ready(
		function() {
			...
			$("#playlist-container").on(
					...
					function(event) {
						var playlistElement = $(this).closest(
								'.dz-playlist-item');
						var playlistName = $(this).html();
						var playlistAlias = playlistElement.data('alias');
						$.get(Routing.generate('_deezer_playlist_songs', {
							'playlistId' : playlistElement.data('id')
						}), function(response) {
							if (response.success == true) {
			...
			$("#playlist-container").on(
					'click',
					'.playPlaylistDeezerBtn',
					function(event) {

						$.get(Routing.generate('_deezer_playlist_songs', {
							'playlistId' : $(this).closest('.dz-playlist-item')
									.data('id')
						}), function(response) {
							if (response.success == true) {
